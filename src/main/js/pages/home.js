...
class HomePage extends React.Component {
	constructor(props) {
		super(props);
		this.state = { futbolistas: [], clubes: [] };
	}
	componentDidMount() {

		client({ method: 'GET', path: '/api/futbolistas' }).done(response => {
			this.setState({ musicos: response.entity._embedded.musicos });
		});

		client({ method: 'GET', path: '/api/clubes' }).done(response => {
	...
	render() {
		return (
			<>
				<h1>TransfermarkFake</h1>

				<div style={  {"width": "100%", "display": "flex"}   }>
					<div style={{"width": "calc(100% / 3)"}}>
						<Titulo entidad="Futbolistas" emoji="🎶" />
						<MusicoList musicos={this.state.futbolistas} />
						<Link to="/nuevo-futbolista">Nuevo Jugador</Link>
					</div>
					<div style={{"width": "calc(100% / 3)"}}>
						<Titulo entidad="Clubes" emoji="👩🏼‍🎤" />
						<BandaList bandas={this.state.clubes} />
						<Link to="/nueva-club">Nuevo Club</Link>
					</div>
				</div>
